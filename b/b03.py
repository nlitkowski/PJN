...
def b03():
    with open("b03-vulgar-words.json", "r") as f:
        vulgar_words = load(f)
    lines = []
    for key in ("roots", "prefixes", "suffixes"):
        vulgar_words[key].sort(key=lambda x: len(x), reverse=True)
    roots = "|".join(vulgar_words["roots"])
    prefixes = "|".join(vulgar_words["prefixes"])
    suffixes = "|".join(vulgar_words["suffixes"])
    with open("b03-test.txt", "r") as f:
        lines = [line.strip() for line in f]
    with open("b03-test-out.txt", "w") as f:
        for line in lines:
            line = sub(
                r"\S*(" + prefixes + r")?(" + roots + r"){1}( " + suffixes + r")?\S*",
                "---",
                line,
            )
            f.write(line + "\n")
...
"""
Zawartość pliku b03-vulgar-words.json
{
  "roots": [
    "huj",
    "cip",
...
"""
Zawartość pliku b03-test.txt (na koniec został zamieszczony artykuł niezawierający przekleństw, aby zademonstrować działanie skryptu)
Poniżej natomiast lista wielu (choc znajac kreatywnosc ludzi na pewno nie wszystkich) przekleństw polskich
'chuj'
'chuja'
'chujek'
'chuju'
...
Więcej: https://www.antyradio.pl/Muzyka/Duperele/10-utworow-rockowych-ktorych-teksty-po-polsku-brzmia-absurdalnie-12381
"""
